...
export class ShopUI {
  ...
  render() {
    ...
    // 每日特惠列（含免費禮）
    body.appendChild(el('div', { class: 'sh-sub', text: '每日特惠（每日 5:00 更新）' }));
    const deals = el('div', { class: 'sh-row' });
    ...
    // 常駐兌換
    body.appendChild(el('div', { class: 'sh-sub', text: '兌換所' }));
    const fixed = el('div', { class: 'sh-row' });
    for (const it of SHOP_ITEMS) fixed.appendChild(this._itemCard(it));
  ...
  _itemCard(item, { free = false, deal = false } = {}) {
    const bought = boughtCount(item.id);
    const soldOut = bought >= item.daily;
    const node = el('div', { class: `sh-item${soldOut ? ' sold' : ''}${deal ? ' deal' : ''}${free ? ' free' : ''}` });
    if (deal) node.appendChild(el('div', { class: 'sh-off', text: `${Math.round((1 - item.off) * 100)}% OFF` }));
    ...
    node.appendChild(el('div', { class: 'sh-limit', text: soldOut ? '今日已售完' : `今日 ${bought}/${item.daily}` }));
