...
//
// ══ 被動類技能四分類（2026-07 定案）══
//   1. 星級里程碑（stats.js STAR_MILESTONES）  2. 進場被動（onEnter：開天氣/場地）
//   3. 光環被動（passives 無條件/自身條件光環 ＋ triggers 事件觸發——同一分類）
//   4. 隊伍技（passives 帶 when.alliesAtLeast：湊滿 X 名系列/種族才生效）
//   生效規則：星級與隊伍技＝進場判定、鎖定到戰鬥結束（死活不影響）；
//            光環＝活體光環（持有者死亡即消失、條件每步重驗）；進場＝一次性。
//
...
export const CARDS = {
  ...
  // ---- 水 ----
  tidecaller: { id: 'tidecaller', name: '潮汐術士', element: 'water', class: 'dps', attackStyle: 'ranged', race: '人', series: ['潮汐'], base: { hp: 495, atk: 95, def: 38 }, growth: { hp: 54, atk: 11, def: 4 }, passives: [{ when: { alliesAtLeast: { count: 4, where: { class: 'dps' } } }, target: 'allAllies', targetWhere: { class: 'dps' }, effects: [{ grant: 'cheatDeath', healPct: 0.5 }] }] }, // 隊伍技：攻擊成員≥4時，所有攻擊成員首次致命傷害免死並回復50%最大生命
  ...
  magmaturtle: { id: 'magmaturtle', onEnter: { weather: 'sunny' }, name: '熔殼巨龜', element: 'fire', class: 'tank', attackStyle: 'melee', race: '獸', series: ['守護者'], base: { hp: 730, atk: 55, def: 79 }, growth: { hp: 82, atk: 6, def: 9 } },
  flarearcher: { id: 'flarearcher', name: '烈焰遊俠', element: 'fire', class: 'dps', attackStyle: 'ranged', basicAttack: { splash: 0.35 }, race: '精靈', series: ['獵團'], base: { hp: 475, atk: 101, def: 38 }, growth: { hp: 52, atk: 12, def: 4 }, passives: [{ target: 'self', effects: [{ stat: 'atk', op: 'mul', basePct: 0.03, perCountOf: { side: 'enemies' } }] }] }, // 濺射普攻示範：火矢波及同排相鄰敵人
  emberwitch: { id: 'emberwitch', name: '燼火魔女', element: 'fire', class: 'dps', attackStyle: 'ranged', race: '人', series: ['燼火'], onEnter: { name: '燼火共鳴', effects: [{ type: 'energy', amount: 50, scope: 'allAllies', where: { series: '燼火' } }] }, base: { hp: 495, atk: 95, def: 38 }, growth: { hp: 54, atk: 11, def: 4 } }, // 定位：引爆燃燒法師——進場「燼火共鳴」給燼火隊友起手能量、絕技「爆燃」全體引爆灼燒
  warbanner: { id: 'warbanner', name: '戰旗軍魂', element: 'fire', class: 'support', attackStyle: 'melee', race: '不死', series: ['鐵壁'], base: { hp: 510, atk: 72, def: 44 }, growth: { hp: 56, atk: 8, def: 4 }, triggers: [{ name: '軍魂不滅', on: 'death', who: 'ally', effects: [{ stat: 'atk', type: 'buff', op: 'mul', value: 1.1, duration: 2, scope: 'allAllies' }] }] }, // 觸發示範：隊友倒下→全軍振奮
  redlion: { id: 'redlion', name: '赤獅騎士', element: 'fire', class: 'tank', attackStyle: 'melee', race: '人', series: ['鐵壁'], onEnter: { name: '烈焰壁壘', effects: [{ type: 'shield', power: 2.0, duration: 3, scope: 'allAllies', where: { element: 'fire' } }] }, base: { hp: 730, atk: 58, def: 73 }, growth: { hp: 82, atk: 6, def: 8 } }, // 進場技「烈焰壁壘」：為火屬隊友張開 200% 攻擊力護盾（持續 3 回合，火隊開局壁壘）

  // ---- 風（+8）----
  stormblade: { id: 'stormblade', name: '蒼雷劍聖', element: 'wind', class: 'dps', attackStyle: 'melee', basicAttack: { everyN: 3, mult: 2.2 }, race: '人', series: ['蒼雷'], base: { hp: 475, atk: 101, def: 38 }, growth: { hp: 52, atk: 12, def: 4 }, passives: [{ when: { selfHpBelow: 0.5 }, target: 'self', effects: [{ stat: 'atk', op: 'mul', value: 1.25 }] }] },
  ...
  tempesthawk: { id: 'tempesthawk', name: '暴風鷹匠', element: 'wind', class: 'dps', attackStyle: 'ranged', race: '精靈', series: ['獵團', '疾風'], onEnter: { name: '疾風先手', effects: [{ type: 'energy', amount: 100, scope: 'self' }] }, base: { hp: 495, atk: 95, def: 38 }, growth: { hp: 54, atk: 11, def: 4 } }, // 進場「疾風先手」：自身入場能量 +100（開場即可放絕技）
  ...
  thundertotem: { id: 'thundertotem', onEnter: { terrain: 'surge' }, name: '雷圖騰師', element: 'wind', class: 'support', attackStyle: 'ranged', race: '獸', series: ['大地'], base: { hp: 510, atk: 72, def: 44 }, growth: { hp: 56, atk: 8, def: 4 } },
  skylancer: { id: 'skylancer', onEnter: { weather: 'gale' }, name: '天嵐槍騎', element: 'wind', class: 'dps', attackStyle: 'melee', race: '龍', series: ['蒼雷'], base: { hp: 495, atk: 95, def: 38 }, growth: { hp: 54, atk: 11, def: 4 } },
  ...
  zephyrmonk: { id: 'zephyrmonk', name: '迅風武僧', element: 'wind', class: 'dps', attackStyle: 'melee', basicAttack: { hits: 2, mult: 0.6 }, race: '人', series: ['疾風'], base: { hp: 495, atk: 95, def: 38 }, growth: { hp: 54, atk: 11, def: 4 }, guardKit: { capPct: 0.2, counterMult: 0.5, lifesteal: 0.3, maxUses: 5 } }, // 被動：單次直傷≤20%總血，超過被夾即全體反擊（50%攻＋回血30%，整場5次）
  ...
  tidesinger: { id: 'tidesinger', name: '潮音歌者', element: 'water', class: 'support', attackStyle: 'ranged', basicAttack: { heal: 0.6 }, race: '人', series: ['潮汐', '星詠'], base: { hp: 510, atk: 72, def: 44 }, growth: { hp: 56, atk: 8, def: 4 }, passives: [{ target: 'allAllies', targetWhere: { series: '潮汐' }, effects: [{ stat: 'dmgTaken', op: 'mul', value: 0.94 }] }] },
  ...
  abysshunter: { id: 'abysshunter', name: '深淵獵手', element: 'water', class: 'dps', attackStyle: 'melee', race: '妖', series: ['深淵'], base: { hp: 495, atk: 95, def: 38 }, growth: { hp: 54, atk: 11, def: 4 }, passives: [{ target: 'columnAllies', effects: [{ stat: 'energyGain', op: 'mul', value: 1.5 }] }] }, // 被動：同直排隊友回氣 +50%
  mistdancer: { id: 'mistdancer', name: '霧舞者', element: 'water', class: 'dps', attackStyle: 'melee', race: '精靈', series: ['霜語'], base: { hp: 475, atk: 101, def: 38 }, growth: { hp: 52, atk: 12, def: 4 } },
  ...
  leviathan: { id: 'leviathan', onEnter: { weather: 'rain' }, name: '滄海巨蛇', element: 'water', class: 'dps', attackStyle: 'melee', race: '龍', series: ['深淵', '潮汐'], base: { hp: 525, atk: 95, def: 36 }, growth: { hp: 57, atk: 11, def: 4 } },
  ...
  lumenfox: { id: 'lumenfox', name: '流光靈狐', element: 'light', class: 'support', attackStyle: 'ranged', race: '妖', series: ['星詠'], base: { hp: 510, atk: 72, def: 44 }, growth: { hp: 56, atk: 8, def: 4 } },
  ...
  boneknight: { id: 'boneknight', onEnter: { terrain: 'erosion' }, name: '白骨騎士', element: 'dark', class: 'tank', attackStyle: 'melee', race: '不死', series: ['影之眷屬', '鐵壁'], base: { hp: 730, atk: 58, def: 73 }, growth: { hp: 82, atk: 6, def: 8 } },
  ...
  voidcaller: { id: 'voidcaller', name: '虛空喚者', element: 'dark', class: 'dps', attackStyle: 'ranged', race: '人', series: ['深淵'], base: { hp: 495, atk: 95, def: 38 }, growth: { hp: 54, atk: 11, def: 4 }, triggers: [{ name: '虛空汲取', on: 'markedHit', crit: true, effects: [{ type: 'energy', amount: 20, scope: 'self' }] }] }, // 觸發：帶虛空烙印的敵人被暴擊 → 自身 +20 能量（被動軸改為此觸發，原深淵隊伍技移除）
  cryptwidow: { id: 'cryptwidow', name: '墓穴寡婦', element: 'dark', class: 'dps', attackStyle: 'melee', race: '不死', series: ['影之眷屬'], base: { hp: 475, atk: 101, def: 38 }, growth: { hp: 52, atk: 12, def: 4 }, triggers: [{ name: '死神收割', on: 'death', who: 'enemy', effects: [{ type: 'energy', amount: 25, scope: 'self' }] }] }, // 觸發示範：敵人倒下→收割能量
  duskwarden: { id: 'duskwarden', name: '薄暮守望', element: 'dark', class: 'tank', attackStyle: 'melee', race: '精靈', series: ['秘林'], base: { hp: 730, atk: 58, def: 73 }, growth: { hp: 82, atk: 6, def: 8 } },
  ...
  fluxreaver: { id: 'fluxreaver', name: '奪流魅影', element: 'dark', class: 'dps', attackStyle: 'melee', race: '妖', series: ['深淵'], base: { hp: 495, atk: 95, def: 38 }, growth: { hp: 54, atk: 11, def: 4 }, passives: [{ target: 'adjacentAllies', effects: [{ stat: 'energyGain', op: 'mul', value: 1.1 }] }] }, // 被動：自身與相鄰隊友回氣 +10%
  ...
  vengefulshade: { id: 'vengefulshade', name: '怨靈劍鬼', element: 'dark', class: 'dps', attackStyle: 'melee', race: '不死', series: ['影之眷屬'], base: { hp: 475, atk: 101, def: 38 }, growth: { hp: 52, atk: 12, def: 4 }, triggers: [{ name: '遺恨爆發', on: 'death', who: 'self', effects: [{ type: 'damage', mult: 2.0, scope: 'allEnemies' }] }] }, // 亡語：死也要拉墊背
  ...
  moonhowler: { id: 'moonhowler', name: '月吼狼王', element: 'wind', class: 'dps', attackStyle: 'melee', basicAttack: { hits: 2, mult: 0.58 }, race: '獸', series: ['大地', '秘林'], base: { hp: 475, atk: 101, def: 38 }, growth: { hp: 52, atk: 12, def: 4 }, passives: [{ when: { alliesAtLeast: { count: 3, where: { race: '獸' } } }, target: 'allAllies', targetWhere: { race: '獸', class: ['support', 'dps'] }, effects: [{ stat: 'maxHp', op: 'mul', value: 2 }] }] }, // 隊伍技：獸族達3名時，獸族輔助與攻擊血量×2

  // ---- 龍 ----
  flamewyrm: { id: 'flamewyrm', name: '曜鱗龍將', element: 'fire', class: 'dps', attackStyle: 'melee', basicAttack: { everyN: 3, mult: 2.4 }, race: '龍', series: ['炎之眷屬', '蒼雷'], base: { hp: 525, atk: 95, def: 36 }, growth: { hp: 57, atk: 11, def: 4 }, onEnter: { name: '劇毒龍息', effects: [{ type: 'dot', power: 0.05, basis: 'targetMaxHp', duration: 2, scope: 'allEnemies' }] } }, // 進場：全體中毒2回合(每跳5%最大生命)；絕技對中毒目標傷害翻倍
  ...
  warchoir: { id: 'warchoir', name: '戰歌指揮', element: 'light', class: 'support', attackStyle: 'ranged', race: '人', series: ['聖歌隊'], base: { hp: 510, atk: 72, def: 44 }, growth: { hp: 56, atk: 8, def: 4 }, passives: [{ when: { alliesAtLeast: { count: 4, where: { class: 'support' } } }, target: 'allAllies', targetWhere: { class: 'support' }, effects: [{ stat: 'maxHp', op: 'mul', value: 1.7 }, { stat: 'def', op: 'mul', value: 1.35 }] }] }, // 聖詠齊鳴：湊 4 輔 → 我方「輔助」最大生命+70%、防禦+35%（續航壁壘）
  bladeoath: { id: 'bladeoath', name: '誓刃盟主', element: 'dark', class: 'dps', attackStyle: 'melee', race: '人', series: ['疾風', '深淵'], base: { hp: 475, atk: 101, def: 38 }, growth: { hp: 52, atk: 12, def: 4 }, passives: [{ when: { alliesOnly: { class: 'dps' } }, target: 'allAllies', effects: [{ element: 'dark' }, { stat: 'dmgTaken', op: 'mul', value: 0.6 }, { stat: 'dodge', op: 'add', value: 0.15 }] }] }, // 殺陣：隊伍只有輸出時(不論人數)全體轉暗屬、承傷-40%、迴避+15%
};
