...
describe('站位範圍 fallback（本排/直排空 → 轉移，效果不浪費）', () => {
  ...
  it('lowestHpAllies：取血量比例最低的 N 名隊友（count）', () => {
    const caster = makeUnit({ team: 0, pos: 1, hp: 1000 });
    const a = makeUnit({ team: 0, pos: 2, hp: 1000 }); a.hp = 100; // 10%
    const b = makeUnit({ team: 0, pos: 3, hp: 1000 }); b.hp = 500; // 50%
    const c = makeUnit({ team: 0, pos: 4, hp: 1000 }); // 100%
    const r = resolveScope('lowestHpAllies', caster, [], ctxFor(caster, [caster, a, b, c], []), { count: 2 });
    expect(r).toEqual([a, b]); // 最低兩名：a(10%)、b(50%)
  });
